...
if __name__ == "__main__":
    ...
    df_infected.to_csv("Israel_infected_prediction.csv", index=False)  # ***
    ...
    ax0 = axs.twinx()
    lns1, = axs.plot(time_scale, quarantined_fit, "-", linewidth=1.5, color='C0', label="quarantined")  # ***
    lns2, = ax0.plot(time_scale, death_fit, "-", linewidth=1.5, color='C5', label="total deaths")
    axs.axvline(x=time_scale[data_points_fitting - 1], linewidth=2, color='r')
    lns3 = axs.scatter(time_scale, quarantined_truth, marker="o", s=12, color='C4', label="actual quarantined")  # ***
    lns4 = ax0.scatter(time_scale, total_deaths_truth, marker="o", s=12, color='C9', label="actual total deaths")  # ***
    ...
    ax0.set_ylabel("Deaths", fontsize=20)
    ax0.tick_params(labelsize=13)
    axs.legend(lns, labels, loc='upper left', prop={"size": 12})  # ***
    plt.savefig("prediction_Israel.png", dpi=400, bbox_inches='tight')
    plt.show()

    # ************************ explanation ****************************
    # daily cases
    fig, axs = plt.subplots(4, figsize=(9, 6), dpi=100)
    axs[0].plot(time_scale, df_cases.iloc[0:, 9], "-", linewidth=1.5, color='g', label="daily cases")  # ***
    # booster
    axs[0].scatter(time_scale[2], 1.5e3, marker="P", color="black", s=14)
    axs[0].text(time_scale[3], 2.8e2, s="PI", color="black", font={'size': 14})
    # the first NPI
    axs[0].scatter(time_scale[5], 2.3e4, marker="v", color='r', s=14)  # for the unvaccinated
    axs[0].text(time_scale[6], 2.1e4, s=r"$C_{1}^{0}$", color='r', font={'size': 14})  # for the unvaccinated
    axs[0].scatter(time_scale[5], 1.6e4, marker="v", color='black', s=14)  # for the vaccinated
    axs[0].text(time_scale[6], 1.4e4, s=r"$C_{1}^{v}$", color='black', font={'size': 14})  # for the vaccinated
    # the second NPI
    axs[0].scatter(time_scale[19], 2.3e4, marker="v", color='r', s=14)  # for the unvaccinated
    axs[0].text(time_scale[20], 2.3e4, s=r"$C_{2}^{0}$", color='r', font={'size': 14})  # for the unvaccinated
    axs[0].scatter(time_scale[19], 1.6e4, marker="v", color='black', s=8)  # for the vaccinated
    axs[0].text(time_scale[20], 1.4e4, s=r"$C_{2}^{v}$", color='black', font={'size': 14})
    # the third NPI
    axs[0].scatter(time_scale[33], 2.3e4, marker="v", color='r', s=14)  # for the unvaccinated
    axs[0].text(time_scale[34], 2.3e4, s=r"$C_{3}^{0}$", color='r', font={'size': 14})  # for the unvaccinated
    # the fourth NPI
    axs[0].scatter(time_scale[65], 1.6e4, marker="v", color='black', s=14)  # for the vaccinated
    axs[0].text(time_scale[66], 1.4e4, s=r"$C_{3}^{v}$", color='black', font={'size': 14})  # for the vaccinated
    ...
    axs[0].legend(bbox_to_anchor=(1.01, 0), loc=3, borderaxespad=0, prop={"size": 10})
    ...
    axs[1].plot(time_scale, vaccine_eff, "-", linewidth=1.5, color='b', label="vacc eff")
    axs[1].plot(time_scale, total_vac, "-", linewidth=1.5, color='g', label="vacc rate")
    axs[1].plot(time_scale, total_booster, "--", linewidth=1.5, color='g', label="bster rate")
    axs[1].scatter(time_scale[2], 0.1, marker="P", color="black", s=14)
    axs[1].text(time_scale[3], 0.05, s=r"start boosters", color="black", font={'size': 14})
    axs[1].set_xlim(datetime.date(starting_time), datetime.date(ending_time))  # ***
    axs[1].set_ylim(0, 1.0)
    axs[1].set_ylabel("Eff/cov", fontsize=14)
    axs[1].legend(bbox_to_anchor=(1.01, 0), loc=3, borderaxespad=0, prop={"size": 10})

    # basic transmission rates
    axs[2].plot(time_scale, betas_zero_one, "-", linewidth=1.5, color='r', label=r"$\beta_{base,1}^{0}$")  # ***
    axs[2].plot(time_scale, betas_zero_two, "-", linewidth=1.5, color='g', label=r"$\beta_{base,2}^{0}$")  # ***
    axs[2].plot(time_scale, betas_vac_one, "-", linewidth=1.5, color='b', label=r"$\beta_{base,1}^{v}$")  # ***
    axs[2].plot(time_scale, betas_vac_two, "-", linewidth=1.5, color='purple', label=r"$\beta_{base,2}^{v}$")  # ***

    # the first NPI
    axs[2].scatter(time_scale[5], 2.0, marker="v", color='r', s=14)  # for the unvaccinated
    axs[2].text(time_scale[6], 1.8, s=r"$C_{1}^{0}$", color='r', font={'size': 14})  # for the unvaccinated
    axs[2].scatter(time_scale[5], 1.4, marker="v", color='black', s=14)  # for the vaccinated
    axs[2].text(time_scale[6], 1.2, s=r"$C_{1}^{v}$", color='black', font={'size': 14})  # for the vaccinated
    # the second NPI
    axs[2].scatter(time_scale[19], 2.0, marker="v", color='r', s=14)  # for the unvaccinated
    axs[2].text(time_scale[20], 1.8, s=r"$C_{2}^{0}$", color='r', font={'size': 14})  # for the unvaccinated
    axs[2].scatter(time_scale[19], 1.4, marker="v", color='black', s=14)  # for the vaccinated
    axs[2].text(time_scale[20], 1.2, s=r"$C_{2}^{v}$", color='black', font={'size': 14})
    # the third NPI
    axs[2].scatter(time_scale[33], 2.0, marker="v", color='r', s=14)  # for the unvaccinated
    axs[2].text(time_scale[34], 1.8, s=r"$C_{3}^{0}$", color='r', font={'size': 14})  # for the unvaccinated
    # the fourth NPI
    axs[2].scatter(time_scale[65], 1.4, marker="v", color='black', s=14)  # for the vaccinated
    axs[2].text(time_scale[66], 1.2, s=r"$C_{3}^{v}$", color='black', font={'size': 14})  # for the vaccinated

    axs[2].set_xlim(datetime.date(starting_time), datetime.date(ending_time))  # ***
    # axs[2].set_xlabel("Days")
    axs[2].set_ylim(0, 2.5)
    axs[2].set_ylabel(r"Basic $\beta$s", fontsize=14)
    axs[2].legend(loc='upper left')  # ***
    axs[2].legend(bbox_to_anchor=(1.01, 0), loc=3, borderaxespad=0, prop={"size": 10})

    # transmission rates
    axs[3].plot(time_scale, trans_zero_one, "-", linewidth=1.5, color='r', label=r"$\beta_{1}^{0}$")  # ***
    axs[3].plot(time_scale, trans_zero_two, "-", linewidth=1.5, color='g', label=r"$\beta_{2}^{0}$")  # ***
    axs[3].plot(time_scale, trans_vac_one, "-", linewidth=1.5, color='b', label=r"$\beta_{1}^{v}$")  # ***
    axs[3].plot(time_scale, trans_vac_two, "-", linewidth=1.5, color='purple', label=r"$\beta_{2}^{v}$")  # ***
    ...
    axs[3].set_xlabel("Days", fontsize=16)
    axs[3].set_ylim(0, 2.0)

    axs[3].set_ylabel(r"Eff $\beta$s", fontsize=14)
    # axs[3].legend(loc='upper left')  # ***
    axs[3].legend(bbox_to_anchor=(1.01, 0), loc=3, borderaxespad=0, prop={"size": 10})
    plt.savefig("transmissionrates_Israel.png", dpi=400, bbox_inches='tight')
    plt.show()
    ...
    axs[3].set_xlim(datetime.date(starting_time), datetime.date(ending_time))  # ***
    # axs[0].set_xlabel("Days")
    axs[3].set_ylim(0, 0.0008)
    axs[3].set_ylabel("death rates")
    axs[3].legend(loc='upper right', prop={"size": 12})  # ***

    plt.savefig("prediction_Israel_rates.png", dpi=400, bbox_inches='tight')
    plt.show()
