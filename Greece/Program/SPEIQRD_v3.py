...
if __name__ == "__main__":
    ...
    init_infected = 0.0003 * total_population  # a constant ***
    ...
    df_infected.to_csv("Greece_infected_prediction_ablation1.csv", index=False)
    ...
    fig, axs = plt.subplots(1, figsize=(9, 6), dpi=100)  # do not change dpi value
    ax0 = axs.twinx()
    lns1, = axs.plot(time_scale, quarantined_fit, "-", linewidth=1, color='C1', label="quarantined (ablation1)")  # ***
    lns2, = ax0.plot(time_scale, death_fit, "-", linewidth=1, color='C6', label="total deaths (ablation1)")
    ...
    plt.savefig("prediction_Greece_ablation1.png", dpi=400, bbox_inches='tight')
    plt.show()
    ...
    axs[1].plot(time_scale, protection_rates, "-", linewidth=1, color='b', label="protection_rates")
    ...
    axs[2].scatter(time_scale[47], betas[46]-0.05, marker="o", c='w', edgecolor="g", s=12)
    axs[2].scatter(time_scale[66], betas[65], marker="o", c='w', edgecolor="g", s=12)
    axs[2].annotate(r"$C_{1}^{v}$", xy=(time_scale[47], betas[47]-0.1), xytext=(time_scale[46], 2.6),
                    arrowprops=dict(arrowstyle="->", color='r'), font={'size': 8}, color='r')
    axs[2].annotate(r"$C_{2}^{v}$", xy=(time_scale[66], betas[66]-0.1), xytext=(time_scale[66], 2.6),
                    arrowprops=dict(arrowstyle="->", color='r'), font={'size': 8}, color='r')
    ...
    plt.savefig("transmissionrates_Greece_ablation1.png", dpi=400, bbox_inches='tight')
    plt.show()
