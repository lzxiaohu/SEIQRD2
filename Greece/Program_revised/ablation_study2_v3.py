...
if __name__ == "__main__":
    ...
    vaccination_rate_prediction = np.ones(prediction_length) * 0.15 / 100.0  # a constant of 0.15
    ...
    df_infected.to_csv("Greece_infected_prediction_ablation2.csv", index=False)  # ***
    ...
    betas_zero, betas_vac = functions.transmissionRateSVEIQRD(para_est[4], NPIs_no_vac, NPIs_vac, para_est[0],
                                                              para_est[1], para_est[5: 8], para_est[8:9])
    ...
    ax0 = axs.twinx()
    lns1, = axs.plot(time_scale, quarantined_fit, "-", linewidth=1, color='C2', label="quarantined (ablation2)")  # ***
    lns2, = ax0.plot(time_scale, death_fit, "-", linewidth=1, color='C7', label="total deaths (ablation2)")
    ...
    plt.savefig("prediction_Greece_ablation2.png", dpi=400, bbox_inches='tight')
    plt.show()
    ...
    print("betas_zero", betas_zero)
    print("betas_vac", betas_vac)
    axs[2].plot(time_scale, betas_zero, "-", linewidth=1, color='r', label=r"$\beta_{base,1}^{0}$")  # ***
    axs[2].scatter(time_scale[47], betas_zero[46]-0.05, marker="o", c='w', edgecolor="g", s=12)
    axs[2].plot(time_scale, betas_vac, "-", linewidth=1, color='b', label=r"$\beta_{base,1}^{v}$")  # ***
    axs[2].scatter(time_scale[47], betas_vac[46]-0.05, marker="o", c='w', edgecolor="b", s=12)
    ...
    axs[2].annotate(r"$C_{1}^{v}$", xy=(time_scale[47], betas_zero[47]-0.1), xytext=(time_scale[46], 2.6),
                    arrowprops=dict(arrowstyle="->", color='r'), font={'size': 8}, color='r')
    axs[2].annotate(r"$C_{1}^{v}$", xy=(time_scale[47], betas_vac[47]-0.1), xytext=(time_scale[46], 2.6),
                    arrowprops=dict(arrowstyle="->", color='r'), font={'size': 8}, color='r')
    axs[2].scatter(time_scale[65], 2.7, marker="v", color='r', s=16)  # for the unvaccinated
    axs[2].annotate(r"$C_{2}^{v}$", xy=(time_scale[66], betas_zero[66]-0.1), xytext=(time_scale[66], 2.6),
                    arrowprops=dict(arrowstyle="->", color='r'), font={'size': 8}, color='r')
    ...
    axs[3].plot(time_scale, trans_vac, "-", linewidth=1, color='b', label=r"$\beta_{1}^{v}$")  # ***
    ...
    plt.savefig("transmissionrates_Greece_ablation2.png", dpi=400, bbox_inches='tight')
    plt.show()