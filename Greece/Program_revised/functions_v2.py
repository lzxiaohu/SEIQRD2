...
def SEIQRD2Fit(x_data, *args):
    ...
    init_NPIs_eff_zero_two = args[11: 15]  # ***
    init_NPIs_eff_vac_one = args[15: 19]  # ***
    init_NPIs_eff_vac_two = args[19: 23]  # ***
    ...
    for ii in range(data_points):
        ...
        if betas_zero_one[ii] <= 0:
            betas_zero_one[ii] = 1000
        if betas_zero_two[ii] <= 0:
            betas_zero_two[ii] = 1000
        if betas_vac_one[ii] <= 0:
            betas_vac_one[ii] = 1000
        if betas_vac_two[ii] <= 0:
            betas_vac_two[ii] = 1000
...
def SEIQRD2FitV2(x_data, *args):
    ...
    # quarantined_zero = x_data[6]
    # quarantined_vac = x_data[7]
    recovered_zero = x_data[3]
    recovered_vac = x_data[4]
    death_zero = x_data[5]
    death_vac = x_data[6]
    alphas = x_data[7]
    lams_zero = x_data[8]
    ...
    phis_zero = x_data[10]
    phis_vac = x_data[11]
    NPIs_no_vac = x_data[12]
    NPIs_vac = x_data[13]
    vaccine_eff = x_data[14]
    pop = x_data[15]
    data_points = len(alphas)
    # set initial values of parameters
    init_beta_zero_one = args[0]
    ...
    exposed_zero = np.zeros(data_points) + args[7] * pop[0]
    exposed_vac = np.zeros(data_points) + args[8] * pop[0]
    infected_zero = np.zeros(data_points) + args[9] * pop[0]
    infected_vac = np.zeros(data_points) + args[10] * pop[0]
    quarantined_zero = np.zeros(data_points) + args[11] * pop[0]
    quarantined_vac = total_quarantined - quarantined_zero

    init_NPIs_eff_zero_one = args[12: 15]  # ***
    init_NPIs_eff_zero_two = args[15: 18]  # ***
    init_NPIs_eff_vac_one = args[18: 21]  # ***
    ...
    for ii in range(data_points):
        ...
        # rule out negative values
        if betas_zero_one[ii] <= 0:
            betas_zero_one[ii] = 1e10
        if betas_zero_two[ii] <= 0:
            betas_zero_two[ii] = 1e10
        if betas_vac_one[ii] <= 0:
            betas_vac_one[ii] = 1e10
        if betas_vac_two[ii] <= 0:
            betas_vac_two[ii] = 1e10
...
def SEIQRD2SimulationV2(x_data, *args):
    """
    implement deterministic version of SEIQRD^2
    :parameter:
    --------
    ...
    for ii in range(data_points):
        ...
        if NPIs_no_vac[ii] == 0 and NPIs_vac[ii] != 0:
            ...
            tt[2] = tt[2] + 1
            # betas_vac_two[ii + init_NPIs_delay_days:] = betas_vac_two[ii] + init_NPIs_eff_vac_two[tt[3]]
            betas_vac_two[ii + init_NPIs_delay_days:] = betas_vac_two[ii + init_NPIs_delay_days - 1] + init_NPIs_eff_vac_two[tt[3]]
            tt[3] = tt[3] + 1
        # rule out negative values
        if betas_zero_one[ii] <= 0:
            betas_zero_one[ii] = 1e10
    ...
    for ii in range(1, data_points):
        ...
        # vaccinated
        vaccinated[ii] = vaccinated[ii - 1] - \
                         betas_vac_one[ii - 1] * (1 - vaccine_eff[ii - 1]) * vaccinated[ii - 1] * infected_zero[
                             ii - 1] / pop \
                         - betas_vac_two[ii - 1] * (1 - vaccine_eff[ii - 1]) * vaccinated[ii - 1] * infected_vac[
                             ii - 1] / pop + \
                         alphas[ii - 1] * pop
        # exposed_zero
        exposed_zero[ii] = exposed_zero[ii - 1] + \
...
def fitV2(init, x_data, y_data, bounds):
    """
    :param init: is a set of initial values of parameters
                  init_beta_zero_one, init_beta_zero_two, init_beta_vac_one, init_beta_vac_two,
                  init_gamma, init_delta, init_NPIs_delay_days,
    ...
    # print("bounds: ", bounds)
    # print("init:", init)
    popt, pcov = optimize.curve_fit(SEIQRD2FitV2, x_data, y_data, p0=init, bounds=bounds, maxfev=1e+7)
    return popt
...
def SPEIQRDFit(x_data, *args):
    ...
    for ii in range(1, data_points):
        ...
        susceptible[ii] = susceptible[ii - 1] + -1 * betas[ii - 1] * susceptible[ii - 1] * infected[ii - 1] / pop + \
                          -1 * alphas[ii - 1] * pop
        if susceptible[ii] < 0:
            print("the %d th day", ii)
            print("susceptible[ii]: ", susceptible[ii])
            print("betas[ii-1]", betas[ii - 1])
            print("susceptible[ii-1]", susceptible[ii - 1])
