...
if __name__ == "__main__":
    ...
    init_exposed_zero = 1000  # *** 1e-4 * total_population
    init_exposed_vac = 1000  # ***
    init_infected_zero = 20  # *** 1606 =  1.8e-4 * total_population
    init_infected_vac = 100  # *** 892 = 1e-4 * total_population
    init_quarantined_zero = 17000  # *** total quarantined about 17844 = 2.0e-3 * total_population
    init_quarantined_vac = 10000  # *** 7700 = df_cases.iloc[0, 16] - init_quarantined_zero
    init_recovered_zero = 4.5e-3 * total_population  # *** total recovered about 6.8e-3
    init_recovered_vac = df_cases.iloc[0, 7] - init_recovered_zero
    init_death_zero = 3.1e-5 * total_population  # *** total death about 4.7e-5
    ...
    betas_zero_one, betas_zero_two, betas_vac_one, betas_vac_two \
        = functions.transmissionRate(para_est[6], NPIs_no_vac, NPIs_vac, para_est[0], para_est[1], para_est[2],
                                     para_est[3], para_est[7: 13], para_est[13:19], para_est[19:25], para_est[25:29])
    ...
    axs[0].plot(time_scale, quarantined_truth, "o", markersize=2, color='b', label="quarantined")  # ***
    ...
    axs[1].plot(time_scale, quarantined_zero_fit, "o", markersize=2, color='g', label="quarantined_zero_fit")  # ***
    axs[1].plot(time_scale, quarantined_vac_fit, "o", markersize=2, color='b', label="quarantined_vac_fit")  # ***
    ...
    #
    print("betas_zero_one", betas_zero_one[-1])
    print("betas_zero_two", betas_zero_two[-1])
    print("betas_vac_one", betas_vac_one[-1])
    print("betas_vac_two", betas_vac_two[-1])
    axs[2].plot(time_scale, betas_zero_one, "-", linewidth=2, color='r', label="betas_zero_one")  # ***
    axs[2].plot(time_scale, betas_zero_two, "-", linewidth=2, color='g', label="betas_zero_two")  # ***
    axs[2].plot(time_scale, betas_vac_one, "-", linewidth=2, color='b', label="betas_vac_one")  # ***
    axs[2].plot(time_scale, betas_vac_two, "-", linewidth=2, color='purple', label="betas_vac_two")  # ***
    axs[2].set_xlim(datetime.date(starting_time), datetime.date(ending_time))  # ***
    axs[2].set_xlabel("Days")
    axs[2].set_ylabel("Transmission rates")
    axs[2].legend(loc='upper left')  # ***

    axs[3].plot(time_scale, trans_zero_one, "-", linewidth=2, color='r', label="trans_zero_one")  # ***
    axs[3].plot(time_scale, trans_zero_two, "-", linewidth=2, color='g', label="trans_zero_two")  # ***
    axs[3].plot(time_scale, trans_vac_one, "-", linewidth=2, color='b', label="trans_vac_one")  # ***
    axs[3].plot(time_scale, trans_vac_two, "-", linewidth=2, color='purple', label="trans_vac_two")  # ***
    axs[3].set_xlim(datetime.date(starting_time), datetime.date(ending_time))  # ***
    axs[3].set_xlabel("Days")
    axs[3].set_ylabel("Transmission rates")
