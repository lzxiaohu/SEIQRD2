...
if __name__ == "__main__":
    ...
    init_exposed_vac = 2044   # *** 1e-4 * total_population; 1044
    ...
    df_infected.to_csv("Austria_infected_prediction_ablation3.csv", index=False)  # ***
    ...
    plt.savefig("prediction_Austria_ablation3.png", dpi=400, bbox_inches='tight')
    plt.show()
    ...
    # the first NPI
    axs[2].scatter(time_scale[2], 1.5, marker="v", color='r', s=8)  # for the unvaccinated
    axs[2].text(time_scale[3], 1.5, s=r"$C_{1}^{0}$", color='r', font={'size': 6})  # for the unvaccinated
    # the second NPI
    axs[2].scatter(time_scale[49], 1.5, marker="v", color='r', s=8)  # for the unvaccinated
    axs[2].text(time_scale[50], 1.5, s=r"$C_{2}^{0}$", color='r', font={'size': 6})  # for the unvaccinated
    # the third NPI
    axs[2].scatter(time_scale[56], 1.5, marker="v", color='r', s=8)  # for the unvaccinated
    axs[2].text(time_scale[57], 1.5, s=r"$C_{3}^{0}$", color='r', font={'size': 6})  # for the unvaccinated
    # the fourth NPI
    axs[2].scatter(time_scale[63], 1.5, marker="v", color='r', s=8)  # for the unvaccinated
    axs[2].text(time_scale[64], 1.5, s=r"$C_{4}^{0}$", color='r', font={'size': 6})  # for the unvaccinated
    # the fifth NPI
    axs[2].scatter(time_scale[70], 1.5, marker="v", color='black', s=8)  # for the vaccinated
    axs[2].text(time_scale[71], 1.5, s=r"$C_{1}^{v}$", color='black', font={'size': 6})  # for the vaccinated

    axs[2].set_xlim(datetime.date(starting_time), datetime.date(ending_time))  # ***
    # axs[2].set_xlabel("Days")
    axs[2].set_ylim(0, 1.8)
    ...
    plt.savefig("transmissionrates_Austria_ablation3.png", dpi=400, bbox_inches='tight')
    plt.show()
