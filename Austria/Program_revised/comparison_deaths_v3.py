...
df_cases = pd.read_csv("Austria_deaths_prediction_total.csv")
...
axs.plot(time_scale, prediction, "-", linewidth=1, color='C5', label="total deaths")  # ***
axs.plot(time_scale, ablation1, "-", linewidth=1, color='C6', label="total deaths (ablation1)")  # ***
axs.plot(time_scale, ablation2, "-", linewidth=1, color='C7', label="total deaths (ablation2)")  # ***
axs.plot(time_scale, ablation3, "-", linewidth=1, color='C8', label="total deaths (ablation3)")  # ***
...
axs.annotate(r"", xy=(time_scale[data_points_fitting - 1], 0.7e3), xytext=(time_scale[data_points_fitting - 10], 0.7e3),
             arrowprops=dict(arrowstyle="<-", color='r'), font={'size': 8}, color='r')
axs.text(time_scale[data_points_fitting - 7], 0.8e3, s=r"Fitting", color='r', font={'size': 8})
axs.annotate(r"", xy=(time_scale[data_points_fitting - 1], 1.5e3), xytext=(time_scale[data_points_fitting + 10], 1.5e3),
             arrowprops=dict(arrowstyle="<-", color='r'), font={'size': 8}, color='r')
axs.text(time_scale[data_points_fitting + 1], 1.6e3, s=r"Prediction", color='r', font={'size': 8})

axs.set_xlim(datetime.date(starting_time), datetime.date(ending_time))  # ***
axs.set_xlabel("Days")
axs.set_ylim(0, 4.5e3)
axs.set_ylabel("Deaths")
axs.legend(loc='upper left', prop={"size": 8})  # ***
plt.savefig("comparison_deaths_Austria.png", dpi=400, bbox_inches='tight')
plt.show()
